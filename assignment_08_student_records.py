...
def add_student(records: list[dict]) -> None:
    ...
    try:
        ...
        print("Error: Please enter a valid integer for scores.")
        return
    if count < 0:
...
def display_all_students(records: list[dict]) -> None:
    if not records:
...
def calculate_average(records: list[dict]) -> None:
    try:
